...
def get_indexed_dataset_(data_prefix, data_impl, skip_warmup):
    ...
    print(f"data_impl : {data_impl}")
    print(f"skip_warmup : {skip_warmup}")
...
def _build_index_mappings(name, data_prefix, documents, sizes,
    ...
    if build_indices and int(os.environ['LOCAL_RANK']) == 0:    ## SageMaker Revised
    ...
    temp_data = glob.glob(os.path.dirname(idx_path['doc']) + '/*')
    print_rank_0(f" > temp_data {temp_data}")
